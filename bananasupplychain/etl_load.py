...
# ── Timestamp-Offset: realistische Supply-Chain-Zeitreihe (in-memory) ─────────
# Iteration 001 → KW2 2026 (05.01.), jede weitere Iteration +7 Tage.
# Innerhalb jeder Iteration werden Events route-spezifisch über ~15 Tage verteilt
# (Plantage Tag 0 → Supermarkt Tag 14-15) mit leichter Zufallsvariation.
# Deterministisch: gleicher Dateiname → gleicher Seed → gleiche Timestamps.
_ITER_BASE = datetime(2026, 1, 5, 6, 0, 0)

# Starttag innerhalb einer Iteration (min, max) je Transportroute
_ROUTE_START_DAYS = {
...
# Reisedauer je Route in Tagen (für estimated_arrival)
_ROUTE_TRAVEL_DAYS = {
...
# Ankunftstag je Supply-Chain-Knoten innerhalb einer Iteration (min, max)
_NODE_ARRIVAL_DAYS = {
...
def _find_route(filename: str) -> str | None:
...
def _find_node(filename: str) -> str | None:
    lower = filename.lower()
...
def _apply_ts_offset(event: dict, filename: str) -> dict:
    """Verteilt Event-Timestamps auf realistische Supply-Chain-Zeiträume (in-memory)."""
    ...
    if et in ("SupplierCreated", "CustomerCreated", "ProductCreated",
        ...
        event["timestamp"] = (base + timedelta(minutes=rng.randint(0, 180))).isoformat()

    elif et == "OrderCreated":
        event["timestamp"] = (base + timedelta(hours=rng.uniform(7, 10))).isoformat()

    elif et == "BatchHarvested":
        event["timestamp"] = (base + timedelta(hours=rng.uniform(10, 15))).isoformat()
        ...
        start_days = rng.uniform(*_ROUTE_START_DAYS.get(route, (0.5, 1.0)))
        ts = base + timedelta(days=start_days, minutes=rng.randint(0, 90))
        event["timestamp"] = ts.isoformat()
        if "estimated_arrival" in event:
            travel = _ROUTE_TRAVEL_DAYS.get(route, 2.0)
            event["estimated_arrival"] = (ts + timedelta(days=travel, hours=rng.uniform(-3, 3))).isoformat()
        ...
        pos_day   = rng.uniform(day_range[0], day_range[0] + travel)
        event["timestamp"] = (base + timedelta(days=pos_day, hours=rng.uniform(0, 20))).isoformat()

    elif et == "NodeProcessed":
        node = _find_node(filename) or event.get("supply_chain_node", "").lower().replace(" ", "_")
        days = rng.uniform(*_NODE_ARRIVAL_DAYS.get(node, (1.0, 2.0)))
        event["timestamp"] = (base + timedelta(days=days, hours=rng.uniform(8, 18))).isoformat()

    elif et == "TransportCompleted":
        node = event.get("arrival_node", "").lower().replace(" ", "_")
        days = rng.uniform(*_NODE_ARRIVAL_DAYS.get(node, (2.0, 3.0)))
        event["timestamp"] = (base + timedelta(days=days, hours=rng.uniform(10, 20))).isoformat()

    elif et == "DeliveryCompleted":
        node = _find_node(filename) or event.get("supply_chain_node", "").lower().replace(" ", "_")
        days = rng.uniform(*_NODE_ARRIVAL_DAYS.get(node, (14.5, 15.5)))
        event["timestamp"] = (base + timedelta(days=days, hours=rng.uniform(8, 18))).isoformat()
