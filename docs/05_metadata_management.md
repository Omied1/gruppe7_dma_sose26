...
### 3.1 Hierarchische Einordnung

Die Skalenniveaus gliedern sich in zwei Obergruppen:

```
Merkmale
├── kategorial
│     ├── nominal    → Kategorien ohne Reihenfolge  (Interpretation: gleich oder ungleich)
│     └── ordinal    → Kategorien mit Reihenfolge   (Interpretation: kleiner, gleich oder größer)
└── metrisch
      └── kardinal
            ├── Intervall  → Differenzen sinnvoll, kein natürlicher Nullpunkt
            └── Verhältnis → Verhältnisse sinnvoll, natürlicher Nullpunkt (0 = Nichts)
```

> **Hinweis zur Implementierung:** In der Datenbank (`meta.columns.scale_level`) werden die englischen Bezeichner `NOMINAL`, `ORDINAL`, `INTERVAL`, `RATIO` als technische Enum-Werte verwendet. Sie entsprechen direkt den deutschen Begriffen der Vorlesung.

### 3.2 Übersicht

| Obergruppe | Untergruppe | Skalenniveau (Vorlesung) | DB-Wert    | Interpretation              | Beispiele aus der Supply Chain                           | Erlaubte Operationen                        |
| ---------- | ----------- | ------------------------ | ---------- | --------------------------- | -------------------------------------------------------- | ------------------------------------------- |
| kategorial | –           | **nominal**              | `NOMINAL`  | gleich oder ungleich        | `delivery_status`, `product_code`, `transport_mode`      | Häufigkeit, Modus, Chi-Quadrat              |
| kategorial | –           | **ordinal**              | `ORDINAL`  | kleiner, gleich oder größer | `delivery_priority` (LOW < NORMAL < HIGH)                | Median, Rang, nichtparametrische Tests      |
| metrisch   | kardinal    | **Intervall**            | `INTERVAL` | Bildung von Differenzen     | `temperature` (°C), `latitude`, `longitude`, Zeitstempel | Mittelwert, Standardabweichung, Differenzen |
| metrisch   | kardinal    | **Verhältnis**           | `RATIO`    | Aussagen zu Verhältnissen   | `quantity`, `unit_price`, `delay_minutes`, `speed_kmh`   | Alle Operationen inkl. Verhältnisse         |

### 3.3 Begründung zentraler Skalenniveaus

**`delivery_priority` → ordinal (`ORDINAL`):**  
Die Werte HIGH, NORMAL, LOW haben eine klare Reihenfolge (HIGH > NORMAL > LOW), aber der Abstand zwischen den Stufen ist nicht gleichmäßig definiert. Daher ordinal, nicht Intervall.

**`temperature` (°C) → Intervall (`INTERVAL`):**  
Temperatur in Celsius hat keinen natürlichen Nullpunkt (0°C ist nicht „keine Temperatur"). Das Verhältnis 20°C/10°C = 2 ist nicht sinnvoll interpretierbar – nur Differenzen sind aussagekräftig. Daher Intervall. (Kelvin wäre Verhältnis.)

**`delay_minutes` → Verhältnis (`RATIO`):**  
0 Minuten Verzögerung bedeutet tatsächlich „keine Verzögerung" – ein natürlicher Nullpunkt. Das Verhältnis 60/30 = „doppelte Verzögerung" ist sinnvoll interpretierbar. Daher Verhältnis.

**`latitude` / `longitude` → Intervall (`INTERVAL`):**  
Geografische Koordinaten (WGS84) haben keinen natürlichen Nullpunkt für Verhältnisaussagen. 0°/0° ist ein willkürlicher Punkt im Golf von Guinea, nicht „keine Position". Nur Differenzen (Abstände) sind sinnvoll. Daher Intervall.

**`product_code`, `supplier_code` → nominal (`NOMINAL`):**  
